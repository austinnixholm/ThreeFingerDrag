...
HFONT normal_font = CreateFont(17, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, TEXT("Segoe UI"));
...
BOOL InitInstance(const HINSTANCE hInstance)
{
	...
	if (!RegisterRawInputDevices()) {
		Popups::DisplayErrorMessage("ThreeFingerDrag couldn't find a precision touchpad device on your system. The program will now exit.");
		return FALSE;
	}
...
bool InitializeGUI() {
	...
	settings_checkbox_hwnd = CreateWindowW(L"button", L"Run ThreeFingerDrag on startup of Windows",
		...
		pos_x, pos_y, SETTINGS_WINDOW_WIDTH - 32, 20, settings_hwnd, (HMENU)ID_RUN_ON_STARTUP_CHECKBOX, NULL, NULL);
	...
	HWND hwnd_trackbar_label = CreateWindowW(WC_STATIC, L"Gesture speed (slower -> faster)",
		WS_CHILD | WS_VISIBLE | SS_LEFT,
		pos_x, pos_y, SETTINGS_WINDOW_WIDTH - margin, label_height, settings_hwnd, NULL, NULL, NULL);
	...
	settings_trackbar_hwnd = CreateWindowW(TRACKBAR_CLASS, NULL,
		...
		pos_x, pos_y, SETTINGS_WINDOW_WIDTH - margin, trackbar_height, settings_hwnd, (HMENU)ID_GESTURE_SPEED_TRACKBAR, current_instance, NULL);
